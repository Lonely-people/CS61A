...
def memo_diff(diff_function):
    ...
    def memoized(typed, source, limit):
        ...
        tmp = (typed, source)
        if tmp in cache:
            if limit <= cache[tmp][1]:
                return cache[tmp][0]
        cur = diff_function(typed, source, limit)
        cache[tmp] = (cur, limit)
        return cur
        # END PROBLEM EC
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    diff = []
    for i in word_list:
        diff.append(diff_function(typed_word, i, limit))
    if min(diff) > limit:
        return typed_word
    else:
        n = len(diff)
...
def furry_fixes(typed, source, limit):
    ...
    def count_diff(s, t, w, lim):
        ...
        n = len(s); m = len(t)
        if s[0] == t[0]:
            return count_diff(s[1:], t[1:], w, lim)
        else:
            return count_diff(s[1:], t[1:], w + 1, lim)
    n = len(typed); m = len(source)
    return count_diff(typed, source, 0, limit) + (abs(n - m) if n != m else 0)
    # END PROBLEM 6
...
def minimum_mewtations(typed, source, limit):
    ...
    if typed[0] == source[0]:
        ...
        rev = 1 + minimum_mewtations(typed[1:], source, limit - 1)
        sub = 1 + minimum_mewtations(typed[1:], source[1:], limit - 1)
        return min(add, rev, sub)
...
def final_diff(typed, source, limit):
    ...
    def count_diff(s, t, lim):
        ...
        n = len(s); m = len(t)
        if n == 0 and m != 0:
            return 1 + count_diff(s, t[1:], lim - 1)
        if n != 0 and m == 0:
            return 1 + count_diff(s[1:], t, lim - 1)
        if s == t:
            return 0
        if s[0] == t[0]:
            return count_diff(s[1:], t[1:], lim)
        elif n > 1 and m > 1:
            if s[0] == t[1] and s[1] == t[0]:
                return 2 + count_diff(s[2:], t[2:], lim - 2)
        add = 1 + count_diff(s, t[1:], lim - 1)
        rev = 1 + count_diff(s[1:], t, lim - 1)
        sub = 1 + count_diff(s[1:], t[1:], lim - 1)
        return min(add, rev, sub)
    return count_diff(typed, source, limit)
    """if limit == 0:
        return 0 if typed == source else 1
    n = len(typed); m = len(source)
    if n == 0 or m == 0:
    ...
    if typed[0] == source[0]:
        ...
        rev = 1 + final_diff(typed[1:], source, limit - 1)
        sub = 1 + final_diff(typed[1:], source[1:], limit - 1)
        return min(add, rev, sub)
    """
...
def time_per_word(words, timestamps_per_player):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    times = []; n = len(timestamps_per_player)
    for i in range(0, n):
        tmp = []
        m = len(timestamps_per_player[i])
...
def fastest_words(match_object):
    ...
    for i in word_indices:
        ...
        for j in player_indices:
            if get_time(match_object, tmp, i) > get_time(match_object, j, i):
                tmp = j
        ans[tmp].append(get_word(match_object, i))
    return ans
    # END PROBLEM 11
